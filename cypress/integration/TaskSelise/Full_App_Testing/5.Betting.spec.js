...
describe('Betting Test Suite', () => {
    ...
    it('Test the display options of Bet', () => {
        cy.get('table tr .btn').contains('Bet').should('be.visible').click()
        cy.url().should('include', 'bet/1')//Assert
        cy.get('div h3').contains('Tottenham Hotspur vs Chelsea').should('be.visible')//Assert
        cy.pause()
    })
    //src > app > bet > bet.component.html
    it('Test if by clicking on Go Back button the user is taken into home page', () => {
        cy.get('table tr .btn').contains('Bet').should('be.visible').click()
        cy.url().should('include', 'bet/1')
    ...
    it.only('Place the bet with an empty value for all the fields', () => {
        cy.get('table tr .btn').contains('Bet').should('be.visible').click()
        cy.url().should('include', 'bet/1')
